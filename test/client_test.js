...
describe('Client', () => {
  before(async () => {
    const client = new Client(config.chavinci);
  ...
  describe('constructor', () => {
    it('should throw an error if network is invalid', () => {
      try {
        new Client(_.defaults({ network: 'foo' }, config.chavinci)); // eslint-disable-line no-new
    ...
    it('should have all the methods listed by `help`', async () => {
      const help = await new Client(config.chavinci).help();
    ...
    it('should accept valid versions', async () => {
      await new Client(_.defaults({ version: '0.15.0.1' }, config.chavinci)).getNetworkInfo();
      await new Client(_.defaults({ version: '0.15.0' }, config.chavinci)).getNetworkInfo();
      await new Client(_.defaults({ version: '0.17.0' }, config.chavinci)).getNetworkInfo();
    });
  });
  ...
  describe('connections', () => {
    describe('general', () => {
      it('should throw an error if timeout is reached', async () => {
        try {
          await new Client(_.defaults({ timeout: 1 }, config.chavinci)).listUnspent();
      ...
      it('should throw an error if a connection cannot be established', async () => {
        try {
          await new Client(_.defaults({ port: 9897 }, config.chavinci)).getDifficulty();
    ...
    describe('ssl', () => {
      it('should use `ssl.strict` by default when `ssl` is enabled', () => {
        const sslClient = new Client(_.defaults({ host: config.chavinciSsl.host, port: config.chavinciSsl.port, ssl: true }, config.chavinci));
      ...
      it('should establish a connection if certificate is self signed but `ca` agent option is passed', async () => {
        const sslClient = new Client(_.defaults({
          ...
          host: config.chavinciSsl.host,
          port: config.chavinciSsl.port,
          ssl: true
        }, config.chavinci));
      ...
      it('should establish a connection if certificate is self signed but `ssl.strict` is disabled', async () => {
        const sslClient = new Client(_.defaults({ host: config.chavinciSsl.host, port: config.chavinciSsl.port, ssl: { enabled: true, strict: false } }, config.chavinci));
        const info = await sslClient.getInfo();
    ...
    describe('authentication', () => {
      it('should throw an error if credentials are invalid', async () => {
        try {
          await new Client(_.defaults({ password: 'biz', username: 'foowrong' }, config.chavinci)).getDifficulty();
        } catch (e) {
      ...
      it('should support username only authentication', async () => {
        const difficulty = await new Client(config.chavinciUsernameOnly).getDifficulty();
